...
def pairwise_distance_l1(point_cloud):
  og_batch_size = point_cloud.get_shape().as_list()[0]
  point_cloud = tf.squeeze(point_cloud)
  if og_batch_size == 1:
    point_cloud = tf.expand_dims(point_cloud, 0)
...
def knn_thres(adj_matrix, k=20, thres=0.5):
  """Get KNN based on the pairwise distance.
  Args:
    pairwise distance: (batch_size, num_points, num_points)
    k: int
  ...
  Returns:
    nearest neighbors: (batch_size, num_points, k)
  """
  og_batch_size = adj_matrix.get_shape().as_list()[0]
  neg_adj = -adj_matrix
  vals, nn_idx = tf.nn.top_k(neg_adj, k=k)

  to_add = tf.range(nn_idx.get_shape()[1])
  to_add = tf.reshape(to_add, [-1, 1])
  to_add = tf.tile(to_add, [1, k]) #[N,k]

  final_nn_idx = []
  for idx in range(og_batch_size):
    idx_vals = vals[idx, :, :]
    idx_nn_idx = nn_idx[idx, :, :]
    mask = tf.cast(idx_vals < -1*thres, tf.int32) # [N, K]
    idx_to_add = to_add * mask
    idx_nn_idx = idx_nn_idx * (1 - mask) + idx_to_add
    final_nn_idx.append(idx_nn_idx)

  return tf.stack(final_nn_idx, axis=0)

def get_local_feature(point_cloud, nn_idx, k=20):
